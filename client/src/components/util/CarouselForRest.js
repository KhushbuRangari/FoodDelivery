...
function CarouselForRest({item}) {
  return (
    ...
    <div className="carousel-inner p-3">
      {item.data.map((items, index) =>{
        ...
        {  items.menuItems.map((img,index)=>{
            ...
            return( <li className='list-group-item ' key={index}><img src={img.imageURL} className="d-block w-100" alt={`Slide ${index + 1}`} width={90} height={180}/>
          ...
          {  items.menuItems.map((img,index)=>{
              ...
              return( <li className='list-group-item ' key={index}> {img.name}
              </li>)
    ...
    <button className="carousel-control-prev" type="button" data-bs-target="#carouselRest" data-bs-slide="prev">
      <span className="carousel-control-prev-icon" aria-hidden="true" />
      <span className="visually-hidden">Previous</span>
    </button>
    <button className="carousel-control-next" type="button" data-bs-target="#carouselRest" data-bs-slide="next">
