...
const Offcanvas = () => {
  ...
  const calculateTotal = () => {
    return cart.reduce((total, item) => total + item.quantity * item.item.price, 0).toFixed(2);
  };
  ...
  return (
    ...
    >
      <div className="offcanvas-header" style={{ backgroundColor: "#f0f0f0", borderBottom: "1px solid #999898" }}>
          ...
          <table className="table table-bordered m-0">
            ...
            <thead>
              ...
              <tr>
                ...
                <th className="text-center align-middle py-3 px-0" style={{ width: 60 }}>
                  Clear Cart
                </th>
              </tr>
            </thead>
            <tbody>
              {cart.map((item) => (
                <tr key={item.item._id}>
                  <td className="">
                  <Link to={`/resturant/${item.restId}`} style={{ textDecoration: "none" }}>
                  ...
                  <td className="text-right font-weight-semibold align-middle p-2">{item.item.price}</td>
                  <td className="align-middle p-2">{item.quantity}</td>
                  <td className="text-right font-weight-semibold align-middle p-2">{(item.quantity * item.item.price).toFixed(2)}</td>
